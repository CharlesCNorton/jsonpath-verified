...
(* We need basic Show instances first *)
...
Fixpoint value_eqb (v1 v2:JSON.value) : bool :=
  ...
  | JSON.JArr xs, JSON.JArr ys => list_eqb value_eqb xs ys
  | JSON.JObject fs1, JSON.JObject fs2 =>
      list_eqb (fun '(k1,v1) '(k2,v2) =>
                  andb (string_eqb k1 k2) (value_eqb v1 v2)) fs1 fs2
  | _, _ => false
  end.
...
(* ---------- How to run comprehensive tests ----------
   ...
   For a full test suite with 1000 tests each:
   
     QuickChick test_suite_with_stats.
     
   For individual extensive tests:
   
     QuickChick extensive_test_linear.
     QuickChick extensive_test_wildcard_obj.
     QuickChick extensive_test_wildcard_arr.
     QuickChick extensive_test_desc.
     QuickChick extensive_test_search.
   ...
   To run all tests at once:
     ...
     QuickChick run_all_tests.

   Command line execution (after extraction):
   
     ./test_jsonpath --num-tests 10000 --max-size 50
*)
