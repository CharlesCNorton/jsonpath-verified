...
Definition selector_leb (s1 s2 : selector) : bool :=
  match s1, s2 with
  | SelName n1, SelName n2 => string_leb n1 n2
  | SelName _, _ => true
  | _, SelName _ => false
  | SelIndex i1, SelIndex i2 => Z.leb i1 i2
  | SelIndex _, _ => true
  | _, SelIndex _ => false
  | SelWildcard, SelWildcard => true
  | SelWildcard, _ => true
  | _, SelWildcard => false
  | SelSlice start1 stop1 step1, SelSlice start2 stop2 step2 =>
      if Z.eqb start1 start2 then
        if Z.eqb stop1 stop2 then Z.leb step1 step2
        else Z.leb stop1 stop2
      else Z.leb start1 start2
  | SelSlice _ _ _, _ => true
  | _, SelSlice _ _ _ => false
  | SelFilter _, SelFilter _ => true
  end.

Fixpoint insert_selector (sel : selector) (sels : list selector) : list selector :=
  match sels with
  | [] => [sel]
  | s :: rest =>
      if selector_leb sel s then sel :: sels
      else s :: insert_selector sel rest
  end.

Fixpoint sort_selectors (sels : list selector) : list selector :=
  match sels with
  | [] => []
  | s :: rest => insert_selector s (sort_selectors rest)
  end.

Fixpoint flatten_segments (segs : list segment) : list segment :=
  match segs with
  | [] => []
  | Child sels1 :: Child sels2 :: rest =>
      flatten_segments (Child (sels1 ++ sels2) :: rest)
  | seg :: rest => seg :: flatten_segments rest
  end.

Definition normalize_segment (seg : segment) : segment :=
  match seg with
  | Child sels => Child (sort_selectors sels)
  | Desc sels => Desc (sort_selectors sels)
  end.

Definition normalize_query (q : query) : query :=
  match q with
  | Query segs =>
      Query (map normalize_segment (flatten_segments segs))
  end.

Lemma insert_selector_permutation :
  forall sel sels,
    Permutation (sel :: sels) (insert_selector sel sels).
Proof.
  intros sel sels.
  induction sels as [|s rest IH]; simpl.
  - apply Permutation_refl.
  - destruct (selector_leb sel s) eqn:E.
    + apply Permutation_refl.
    + simpl. apply perm_swap.
Qed.

Lemma sort_selectors_permutation :
  forall sels,
    Permutation sels (sort_selectors sels).
Proof.
  induction sels as [|s rest IH]; simpl.
  - apply Permutation_refl.
  - eapply Permutation_trans.
    + apply perm_skip. exact IH.
    + apply insert_selector_permutation.
Qed.

Lemma flatten_segments_preserves_app :
  forall segs1 segs2,
    flatten_segments (segs1 ++ segs2) =
    flatten_segments (flatten_segments segs1 ++ segs2).
Proof.
  induction segs1 as [|seg1 segs1' IH]; intros segs2; simpl.
  - reflexivity.
  - destruct seg1 as [sels1 | sels1]; destruct segs1' as [|seg2 segs1'']; simpl.
    + reflexivity.
    + destruct seg2 as [sels2 | sels2]; simpl.
...
Lemma flatten_segments_idempotent :
  forall segs,
    flatten_segments (flatten_segments segs) = flatten_segments segs.
Proof.
  induction segs as [|seg segs' IH]; simpl.
  - reflexivity.
  - destruct seg as [sels|sels]; simpl.
    + destruct (flatten_segments segs') as [|seg' rest'] eqn:E; simpl.
      * reflexivity.
      * destruct seg' as [sels'|sels']; simpl.
        ** rewrite <- E. rewrite <- IH. simpl. rewrite E. reflexivity.
        ** reflexivity.
    + destruct (flatten_segments segs') as [|seg' rest'] eqn:E; simpl.
      * reflexivity.
      * destruct seg' as [sels'|sels']; simpl.
...
Lemma child_on_node_app :
  forall sels1 sels2 n results,
    child_on_node (sels1 ++ sels2) n results ->
    exists results1 results2,
      child_on_node sels1 n results1 /\
      child_on_node sels2 n results2 /\
      Permutation results (results1 ++ results2).
Proof.
  intros sels1 sels2 n results Hchild.
  revert sels2 results Hchild.
  induction sels1 as [|sel1 sels1' IH]; intros sels2 results Hchild; simpl in Hchild.
  - exists [], results. split; [constructor | split; [exact Hchild | apply Permutation_refl]].
  - inversion Hchild as [|? ? selector_results rest_results Hsel Hrest Heq]; subst; clear Hchild.
    specialize (IH sels2 rest_results Hrest) as [results1 [results2 [Hchild1 [Hchild2 Hperm]]]].
    exists (selector_results ++ results1), results2.
    split.
    + constructor; [exact Hsel | exact Hchild1].
    + split; [exact Hchild2 |].
      simpl. rewrite app_assoc.
      eapply Permutation_trans; [| exact Hperm].
      apply Permutation_app; [apply Permutation_refl | apply Permutation_refl].
Qed.

Lemma child_on_node_join :
  forall sels1 sels2 n results1 results2,
    child_on_node sels1 n results1 ->
    child_on_node sels2 n results2 ->
    child_on_node (sels1 ++ sels2) n (results1 ++ results2).
Proof.
  intros sels1 sels2 n results1 results2 H1 H2.
  induction H1 as [| sel sels selector_results rest_results Hsel Hrest IH]; simpl.
  - exact H2.
  - constructor; [exact Hsel | apply IH; exact H2].
Qed.

Lemma flatten_eval_seg_two_child :
  forall sels1 sels2 n results,
    eval_seg (Child (sels1 ++ sels2)) n results ->
    eval_seg (Child sels1) n (concat (map (fun n' => seg_exec_nf (Child sels2) n') results)).
Proof.
  intros sels1 sels2 n results Heval.
  inversion Heval as [? ? ? Hchild |]; subst; clear Heval.
  destruct (child_on_node_app sels1 sels2 n results Hchild) as [results1 [results2 [H1 [H2 Hperm]]]].
  eapply EvalSegChild.
  exists (map (fun n' => seg_exec_nf (Child sels2) n') results1).
  split.
  - clear Hperm H2. induction H1 as [| sel sels selector_results rest_results Hsel Hrest IH]; simpl.
    + constructor.
    + constructor; [apply sel_exec_nf_sound; reflexivity | exact IH].
  - clear H1 results Hperm.
    induction H2 as [| sel sels selector_results rest_results Hsel Hrest IH]; simpl.
    + reflexivity.
    + rewrite concat_app. rewrite IH. reflexivity.
Qed.

Theorem flatten_segments_preserves_exec :
  forall segs ns,
    forallb segment_child_only segs = true ->
    segs_exec_nf (flatten_segments segs) ns = segs_exec_nf segs ns.
Proof.
  induction segs as [|seg segs' IH]; intros ns Hok; simpl.
  - reflexivity.
  - apply Bool.andb_true_iff in Hok as [Hseg Hok'].
    destruct seg as [sels1 | sels1]; simpl in Hseg; [| discriminate Hseg].
    destruct segs' as [|seg2 segs'']; simpl.
    + reflexivity.
    + destruct seg2 as [sels2 | sels2]; simpl.
      * apply Bool.andb_true_iff in Hok' as [Hseg2 Hok''].
        simpl in Hseg2.
        rewrite IH by (simpl; rewrite Bool.andb_true_iff; split; [reflexivity | exact Hok'']).
        unfold segs_exec_nf, segs_exec_impl.
        fold (segs_exec_impl Exec.sel_exec_nf).
        fold segs_exec_nf.
        f_equal. apply map_ext. intros n. unfold seg_exec_nf, seg_exec_impl, child_on_node_impl.
        reflexivity.
      * rewrite IH by exact Hok'. reflexivity.
Qed.

Lemma segment_child_only_normalize :
  forall seg,
    segment_child_only seg = true ->
    segment_child_only (normalize_segment seg) = true.
Proof.
  intros seg Hok.
  destruct seg as [sels | sels]; simpl in *; exact Hok.
Qed.

Theorem normalize_query_preserves_exec :
  forall q J,
    query_child_only q = true ->
    eval_exec_nf (normalize_query q) J = eval_exec_nf q J.
Proof.
  intros [segs] J Hok; simpl in *.
  unfold eval_exec_nf, normalize_query. simpl.
  rewrite <- flatten_segments_preserves_exec by exact Hok.
  f_equal. f_equal. apply map_ext. intro seg.
  destruct seg as [sels | sels]; reflexivity.
Qed.

Definition step_eq_dec (s1 s2 : step) : {s1 = s2} + {s1 <> s2}.
Proof.
  decide equality; apply string_dec + apply Z.eq_dec.
Defined.

Definition path_eq_dec : forall p1 p2 : path, {p1 = p2} + {p1 <> p2}.
Proof.
  decide equality. apply step_eq_dec.
Defined.
...
Definition node_eq_dec (n1 n2 : node) : {n1 = n2} + {n1 <> n2}.
Proof.
  decide equality; [apply value_eq_dec | apply path_eq_dec].
Defined.

Theorem query_semantic_equivalence_decidable :
  forall q1 q2 J,
    query_child_only q1 = true ->
    query_child_only q2 = true ->
    { eval_exec_nf q1 J = eval_exec_nf q2 J } +
    { eval_exec_nf q1 J <> eval_exec_nf q2 J }.
Proof.
  intros q1 q2 J Hok1 Hok2.
  destruct (list_eq_dec node_eq_dec (eval_exec_nf q1 J) (eval_exec_nf q2 J)) as [Heq | Hne].
  - left; exact Heq.
  - right; exact Hne.
Defined.

Example normalization_equivalence_example :
  let q1 := Query [Child [SelName "users"]; Child [SelIndex 0]; Child [SelName "name"]] in
  let q2 := Query [Child [SelName "users"; SelIndex 0; SelName "name"]] in
  eval_exec_nf q1 acme_db_json = eval_exec_nf q2 acme_db_json.
Proof.
  reflexivity.
Qed.
...
Separate Extraction
  normalize_query
  query_semantic_equivalence_decidable
  flatten_segments
  sort_selectors.
